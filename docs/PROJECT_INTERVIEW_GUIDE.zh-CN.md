...
| `analyst` | 或许是分析舆情数据并提出claim | 待定义 | 无 | 4 | **[仅定义契约]** |
...
Forum Host + Forum Collaboration Skill
    ↓
生成本轮 Research Agenda
分析目标、证据缺口、实例配置、Skill Bundle、Tool Set、预算
    ↓
LangGraph 动态 fan-out
    ↓
┌────────────────────┬────────────────────┬────────────────────┐
│ Analyst Instance A │ Analyst Instance B │ Analyst Instance N │
│ 官方叙事分析       │ 社媒观点分析       │ 历史对比/冲突检查  │
└────────────────────┴────────────────────┴────────────────────┘
    ↓
受限工具循环
    ↓
┌────────────────┬───────────────────┬───────────────────┬────────────────────┐
│ Web Search     │ Evidence Database │ TikHub API        │ 内置 ML Tools      │
│ 网页材料       │ 历史证据与报告    │ 有界社媒样本      │ 情感/立场/聚类等   │
└────────────────┴───────────────────┴───────────────────┴────────────────────┘
    ↓
Tool Permission Check + Input Schema Validation
    ↓
Typed Tool Output
    ↓
Evidence / Artifact Normalization
    ↓
┌─────────────────────────────┬──────────────────────────────┐
│ EvidenceRecord              │ DerivedEvidence              │
│ 原始网页、帖子、评论等      │ ML 标签、聚类、统计结果等    │
│ stable evidence_id          │ model/version/input IDs      │
└─────────────────────────────┴──────────────────────────────┘
    ↓
Evidence Store / Evidence Database
    ↓
HanLP Sentence Spliter
    ↓
程序依据原始 offset 截取 SourceSpanCandidate[]
    ↓
各 Analyst 基于受限证据空间生成 AnalysisMemo
    ↓
┌────────────────────────────────────────────────────────────┐
│ AnalysisMemo                                               │
│ executive_summary                                          │
│ sections[]                                                 │
│ claim_ledger[]                                             │
│ evidence_ids[] / derived_artifact_ids[]                    │
│ limitations[] / conflicts[] / open_questions[]             │
└────────────────────────────────────────────────────────────┘
    ↓
Memo-level Preliminary Gate
    ↓
┌────────────────────────────────────────────────────────────┐
│ Citation Existence                                         │
│ Quote Provenance                                           │
...
│ Derived Artifact Provenance Validation                     │
└────────────────────────────────────────────────────────────┘
    ↓
Forum Host fan-in
    ↓
比较各 AnalysisMemo 的共识、冲突、遗漏和开放问题
    ↓
┌───────────────────────────────┐
│ 是否需要下一轮研究？          │
└───────────────────────────────┘
...
生成补证/反方/专题任务                          达到终止条件
    ↓                                             ↓
新一轮 Analyst fan-out                   长上下文 Report Agent
    ↑                                             ↓
    └────────────── Forum Loop ─────────── DraftReport
                                                  +
                                         Final Claim Ledger
                                                  ↓
                                  批量 Claim Extraction / Grouping
                                  按 memo、类型、scope、evidence bundle
                                                  ↓
┌─────────────────────────────────────────────────────────────────────┐
│                    Final Evidence Control Gate                      │
│                                                                     │
│ 1. Citation Existence                                               │
│    evidence_id / artifact_id 是否真实存在                           │
│                                                                     │
│ 2. Quote Provenance                                                 │
│    direct quote 和 supporting span 是否为原文连续片段               │
│                                                                     │
│ 3. Semantic Support                                                 │
│    evidence 是否在语义上支持 factual/opinion claim                  │
│                                                                     │
│ 4. Statistical Recalculation                                        │
│    样本数、比例、排序和 ML 聚合结果能否由程序重新计算                │
│                                                                     │
│ 5. Scope Validation                                                  │
│    平台、时间窗口、样本范围是否被扩大                               │
│                                                                     │
│ 6. Contradiction Check                                               │
│    是否存在未处理的反证、来源冲突或替代解释                         │
│                                                                     │
│ 7. Assessment Validation                                             │
│    evaluator 输出、span、引用范围和 schema 是否合规                 │
└─────────────────────────────────────────────────────────────────────┘
                                                  ↓
                                      SupportAssessmentBatch
                                                  ↓
┌────────────────────────────┬───────────────────────────────────────┐
│ 全部关键 Claim 通过        │ 存在未通过的关键 Claim               │
└────────────────────────────┴───────────────────────────────────────┘
              ↓                                      ↓
      Verified Final Report                定向 Repair Request
              ↓                                      ↓
┌──────────────────────────┐              返回对应 Analyst 或 Report Agent
│ report.md                │                       ↓
│ verification.json        │              缩小 scope / 补证 / 删除主张
│ evidence store           │                       ↓
│ analysis memos           │              再次进入批量验证
│ derived artifacts        │
│ sanitized trace          │
└──────────────────────────┘
...
超过轮次、成本或修复上限
          ...
          Run Rejected
              ↓
保留 evidence、memos、assessments 和 trace
不发布未经验证的最终报告
```
...
### Q22.5：证据控制机制不就是“字符串匹配”吗？

**参考回答**
LLM 不负责创造证据，只负责在程序提供的证据空间里做选择。

老师如果问“别人为什么不这么做”，你可以这么答：

因为很多 demo 型项目把引用控制写在 prompt 里，比如要求模型“请基于来源回答”“请不要编造引用”。但 LLM 本质上还是在自由生成字符串，它可能复制错、改写、拼接，甚至生成不存在的评论。我这里的思路是把引用生成权从模型手里拿回来：工具返回材料后，程序先保存 Evidence，并从 Evidence 中生成候选引用；模型只能选择候选 ID，不能自己写引用。这样最后报告里的 claim 是程序从 Evidence 里物化出来的，而不是模型临场编出来的。
...
就这个项目而言，有很多边界情况都需要我们去设计和磨细节，比如给他写一个判断证据是否支持的词表和参考表，工业中可能有一些业务相关的判断标准也需要我们去慢慢磨，这些细节的东西其实ai做的是很随意的。
